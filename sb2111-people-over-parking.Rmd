...
1. **Route-level trip counting:** For every bus route, count unique trips by direction (when available) during AM and PM peaks
2. **Shape-level aggregation:** Group routes by their GTFS shape_id (which represents the specific path a route follows)
   - Multiple routes sharing the same shape_id are running on the same street
   - Sum trips across all routes using each shape, within each direction
   - Calculate combined frequency for each direction/peak combination
3. **Qualification:** A shape qualifies as a corridor if **ANY** direction in **ANY** peak has combined frequency ≤15 minutes
4. **Geometry conversion:** Qualifying shapes are converted from GTFS point sequences to LINESTRING geometries
5. **Buffering:** Qualified route geometries are buffered by **680 feet** (660 ft = 1/8 mile from curb + 20 ft half street width) and clipped to the Illinois boundary

**Key advantages of shape-based aggregation:**

- **Natural grouping**: GTFS shape_id already identifies routes on the same street - no complex geometric intersection needed
- **Accurate**: Uses actual vehicle paths from GTFS, not approximations
- **Efficient**: Simple aggregation instead of geometric segmentation
- **Clear interpretation**: If Route A and Route B use shape "Main_St_NB", they're on the same street

**Example:**
- Route 12 (DeKalb): 3 AM trips, uses shape "route_12_shape_1"
- Route 8 (DeKalb): 5 AM trips, also uses shape "route_12_shape_1"
- Combined: 8 AM trips → 120 min / 8 trips = **15 minute frequency** → Qualifies!

...
# Download and extract GTFS data for all agencies (Chicago + St. Louis + Champaign-Urbana + Rockford)
...
rmtd_dir <- download_and_extract_gtfs("rmtd", "https://rmtd.org/rmtdgtfs/GTFS_FILES.zip")
...
rmtd_data <- read_normalize_gtfs("rmtd", rmtd_dir)
...
all_stops <- rbindlist(list(cta_data$stops, pace_data$stops, metra_data$stops, metro_stl_data$stops, cumtd_data$stops, rmtd_data$stops), fill = TRUE, use.names = TRUE)
all_routes <- rbindlist(list(cta_data$routes, pace_data$routes, metra_data$routes, metro_stl_data$routes, cumtd_data$routes, rmtd_data$routes), fill = TRUE, use.names = TRUE)
all_trips <- rbindlist(list(cta_data$trips, pace_data$trips, metra_data$trips, metro_stl_data$trips, cumtd_data$trips, rmtd_data$trips), fill = TRUE, use.names = TRUE)
all_stop_times <- rbindlist(list(cta_data$stop_times, pace_data$stop_times, metra_data$stop_times, metro_stl_data$stop_times, cumtd_data$stop_times, rmtd_data$stop_times), fill = TRUE, use.names = TRUE)
...
all_calendar <- rbindlist(list(cta_data$calendar, pace_data$calendar, metra_data$calendar, metro_stl_data$calendar, cumtd_data$calendar, rmtd_data$calendar), fill = TRUE, use.names = TRUE)

# Combine calendar_dates from all agencies (important for agencies like CUMTD)
# Some agencies use calendar_dates.txt exclusively to define service
...
for (data in list(cta_data, pace_data, metra_data, metro_stl_data, cumtd_data, rmtd_data)) {
  ...
  if ("date" %in% names(data$calendar_dates)) {
    data$calendar_dates[, date := as.character(date)]
  }

  # Ensure all expected columns exist
  for (col in standard_calendar_dates_cols) {
    if (!col %in% names(data$calendar_dates)) {
      if (col == "exception_type") {
        data$calendar_dates[, (col) := as.integer(NA)]
      } else {
        data$calendar_dates[, (col) := as.character(NA)]
      }
    }
  }

  # Reorder columns to standard order
  existing_cols <- intersect(standard_calendar_dates_cols, names(data$calendar_dates))
  setcolorder(data$calendar_dates, existing_cols)
}

all_calendar_dates <- rbindlist(list(cta_data$calendar_dates, pace_data$calendar_dates, metra_data$calendar_dates, metro_stl_data$calendar_dates, cumtd_data$calendar_dates, rmtd_data$calendar_dates), fill = TRUE, use.names = TRUE)
...
# Identify weekday services (supports both calendar.txt and calendar_dates.txt)
# This is critical for agencies like CUMTD that use calendar_dates.txt exclusively
...
# Rockford area: Winnebago County (201)
...
rmtd_hubs_union <- if(nrow(rmtd_hub_buffers) > 0) st_intersection(st_union(rmtd_hub_buffers), illinois_boundary) else st_sfc(crs = 4326)
...
agency_pal <- colorFactor(
  palette = c("#009CDE", "#814C9E", "#E31837", "#00A651", "#FF6600", "#FFD700"),  # CTA blue, Pace purple, Metra red, Metro STL green, MTD orange, RMTD gold
...
map <- leaflet() %>%
  ...
  addLegend(
    ...
    colors = c("purple", "#009CDE", "#814C9E", "#E31837", "#00A651", "#FF6600", "#FFD700", "#FF8C00"),
...
rmtd_hub <- bus_hub_summary[agency == "rmtd"]
...
rmtd_corridor <- corridor_summary[agency == "rmtd"]

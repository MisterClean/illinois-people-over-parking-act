...
- Illinois state boundary via tigris package

**Counties Covered:**

The analysis includes **14 Illinois counties** across major urban areas:
- **Chicago Metro:** Cook, DuPage, Kane, Lake, McHenry, Will
- **Metro-East:** St. Clair
- **Central Illinois:** Champaign, McLean, Peoria, Sangamon, Macon
- **Northern Illinois:** Winnebago, DeKalb, Rock Island
- **Western Illinois:** Knox, McDonough
...
- **Bus Hubs:** `r sum(bus_hub_summary$unique_clusters)`
  - CTA: `r cta_hub$unique_clusters`
  - Pace: `r pace_hub$unique_clusters`
  - Metro STL: `r if(nrow(metro_stl_hub) > 0) metro_stl_hub$unique_clusters else 0`
  - CUMTD: `r if(nrow(cumtd_hub) > 0) cumtd_hub$unique_clusters else 0`
  - RMTD: `r if(nrow(rmtd_hub) > 0) rmtd_hub$unique_clusters else 0`

...
**Update:** The People Over Parking Act in (SB2111)[has been signed into law by Governor Pritzker](https://gov-pritzker-newsroom.prezly.com/fact-sheet-northern-illinois-transit-authority-act-sb-2111) and will take effect on June 1, 2026.
...
The People Over Parking Act prohibits local governments from imposing or enforcing minimum automobile parking requirements on development projects located near frequent transit service. The law applies to two types of transit-accessible areas:

**Qualifying Areas:**

-   **Transit Hubs** (1/2 mile radius): Rail stations, ferry terminals served by transit, or intersections where 2 or more bus routes meet with combined service every 15 minutes or less during peak commute periods
-   **Transit Corridors** (1/8 mile buffer): Streets served by one or more bus routes with combined service every 15 minutes or less during peak commute periods

Developers may still voluntarily build parking, and municipalities retain authority to regulate on-street parking, set maximum parking limits, and require bicycle parking. The law does not apply to hotels, motels, or other transient lodging.

**Qualification Criteria (as implemented in this analysis):**
...
# Convert shapes to linestrings for map display (suppress console output)
invisible(capture.output({
  all_shapes_sf <- convert_shapes_to_linestrings(all_shapes)
}))

# Create the interactive map with all layers (suppress console output)
invisible(capture.output({
...
-   To ensure frequency calculations reflect actual service levels, we select **representative weekday service(s)** for each agency using a tiered approach:
-   **Primary method**: Use `calendar.txt` weekday services (Monday-Friday flags all set to 1)
-   **Multi-service handling**: If an agency has multiple weekday services covering different routes (e.g., PACE with regional divisions), ALL weekday services are included to ensure complete route coverage
-   **Fallback method**: If no weekday services in calendar.txt, find services running on a representative Tuesday using `calendar_dates.txt`
...
**Stop Clustering (Hubs and Corridors):**
...
-   Clustering uses a **connected components algorithm** (breadth-first search) for deterministic, reproducible results
-   Corridors also use stop clustering to create edges between consecutive clusters along trip paths
...
-   If GTFS `direction_id` data is available, frequency is calculated by direction for **both hubs and corridors**:
    - **Hubs**: Frequency calculated per cluster per direction
    - **Corridors**: Frequency calculated per edge per direction
...
5. **Fallback geometry:** If shape extraction fails (no shape available, invalid geometry), use straight line between cluster centroids
6. **Qualification:** Edges qualify if combined frequency ≤15 minutes in any direction/peak
7. **Buffering:** Qualified edges are buffered by **680 feet** (660 ft = 1/8 mile + 20 ft half street width) and clipped to Illinois boundary
